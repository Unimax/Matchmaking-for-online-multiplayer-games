...
func runScenario(ctx context.Context, cfg config.View, name string, update updater.SetFunc, currentMMR int) int {
		...
		req := &pb.CreateTicketRequest{
			Ticket: &pb.Ticket{
				Properties: structs.Struct{
					...
					"MMR":       structs.Number(float64(currentMMR)),
	...
	//////////////////////////////////////////////////////////////////////////////
	rank := int((rand.Int63() % 10) + 1)
	updateMMR := (5 - rank) * 5
	s.Status = fmt.Sprintf("Playing Game (Sleeping for 20 sec...) got rank: %d ,MMR gain/lose: %d", rank, updateMMR)
	s.Assignment = assignment
	update(s)

	time.Sleep(time.Second * 20)
	return currentMMR + updateMMR
}
